...
export default function Students() {
  ...
  return (
    <div className="container">
      ...
      {showAdd && (
        <form className="mb-2" style={{ maxWidth: 500, margin: "0 auto" }} onSubmit={e => { e.preventDefault(); addStudent(); }}>
        <label>Name<input name="name" placeholder="Name" value={form.name} onChange={handleChange} required /></label>
        <label>Parent Name<input name="parentName" placeholder="Parent Name" value={form.parentName} onChange={handleChange} required /></label>
        <label>Parent Phone<input name="parentPhone" placeholder="Parent Phone" value={form.parentPhone} onChange={handleChange} required /></label>
        <label>Address<input name="address" placeholder="Address" value={form.address} onChange={handleChange} required /></label>
        <label>Age<input name="age" type="number" placeholder="Age" value={form.age} onChange={handleChange} required /></label>
        <label>Gender
        ...
        <button className="btn" type="submit" style={{ alignSelf: "center", width: 120 }}>➕ Add</button>
        </form>
  )}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "1.5rem", justifyContent: "center", margin: "2rem 0" }}>
        {students.map((s) => {
          ...
          return (
            <div key={s.id} className="card" style={{ width: 220, minHeight: 340, position: "relative", boxShadow: "0 2px 12px rgba(44,62,80,0.10)", background: "#f4f8fb", border: isEditing ? "2px solid #2d6cdf" : "2px solid #eee" }}>
              {isEditing ? (
                <form onSubmit={e => { e.preventDefault(); editStudent(); }} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <label>Age<input name="age" type="number" value={editForm.age} onChange={e => setEditForm({ ...editForm, age: e.target.value })} required /></label>
                  <label>Parent Name<input name="parentName" type="text" value={editForm.parentName} onChange={e => setEditForm({ ...editForm, parentName: e.target.value })} required /></label>
                  <label>Parent Phone<input name="parentPhone" type="text" value={editForm.parentPhone} onChange={e => setEditForm({ ...editForm, parentPhone: e.target.value })} required /></label>
                  ...
                  <label>Gender
                    <select name="gender" value={editForm.gender} onChange={e => setEditForm({ ...editForm, gender: e.target.value })} required>
                      <option value="">Select Gender</option>
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                    </select>
                  ...
                  <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
                    ...
                    <button className="btn" type="button" title="Cancel" style={{ background: "#e74c3c" }} onClick={() => { setEdit(false); setEditForm({}); }}>❌</button>
                  </div>
                  {s.photo && (
                    <img src={`http://localhost:5000/${s.photo.replace(/\\/g, "/")}`} alt="student" width="80" style={{ alignSelf: "center", borderRadius: 8, marginTop: 8 }} />
                  )}
                </form>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                  <div style={{ width: 90, height: 90, borderRadius: "50%", overflow: "hidden", marginBottom: 10, border: "3px solid #fff", boxShadow: "0 1px 6px rgba(44,62,80,0.10)" }}>
                    {s.photo ? (
                      <img src={`http://localhost:5000/${s.photo.replace(/\\/g, "/")}`} alt="student" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                    ) : (
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
                    <button className="btn" title="Edit" style={{ background: "#f1c40f", color: "#fff" }} onClick={() => edittingForm(s)}>✏️</button>
                    <button className="btn" title="Remove" style={{ background: "#e74c3c", color: "#fff" }} onClick={() => removeStudent(s.id)}>🗑️</button>
                  </div>
                </div>
              )}
            </div>
